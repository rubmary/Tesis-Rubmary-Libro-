...
\begin{definition}[{\cite[p.~7]{bib:handbook-blai}}] Una \textbf{estrategia mixta} del jugador $i$, denotada con $\sigma_i$, es una distribución de probabilidad sobre el conjunto $S_i$; es decir $\sigma_i \in \Delta(S_i)$. Se denota con $\sigma_i(s_i)$ la probabilidad de que el jugador $i$ elija la acción $s_i \in S_i$. 
\end{definition}

\begin{definition}[{\cite[p.~7]{bib:handbook-blai}}]
...
\begin{definition}[{\cite[p.~8]{bib:handbook-blai}}]
...
\begin{definition}[{\cite[p.~11]{bib:handbook-blai}}]
...
En el juego piedra, papel o tijera, la ganancia esperada del jugador $i$ cuando utiliza la estrategia $\sigma = (\sigma_1, \sigma_2)$, viene dada por (Teorema~\ref{theo:ganancia-esperada}):
\begin{alignat}{1}
...
\begin{definition}[\cite{bib:nash}]
...
En RPS existe un único equilibrio de Nash (ver Apéndice \ref{apex:chapter:programacion-lineal}), que ocurre cuando $\sigma^*_1 = \sigma^*_2 = \left(\frac{1}{3}, \frac{1}{3}, \frac{1}{3} \right)$. En efecto, note que si el jugador $i$ utiliza $\sigma^*_i$, para cualquier estrategia $\sigma_{-i}$, la ganancia del jugador $-i$ viene dada por:
\begin{alignat}{1}
...
\begin{alignat}{1}
u_1(\sigma_1, \sigma_2)\ =\ 2 \left( \frac{1}{3} \right) \theta_1 + \left(1-\frac{1}{3} \right)\theta_2\ =\ \frac{2}{3}(\theta_1 + \theta_2)\ =\ \frac{2}{3} \,.
\end{alignat}
